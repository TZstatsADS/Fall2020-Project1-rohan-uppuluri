...
## Introduction
The Republican party tends to believe that the government should play a smaller role in people's lives, especially in the economy (https://www.britannica.com/topic/Republican-Party), while the Democratic party supports the idea of a larger government that provides more aid to the people (https://www.britannica.com/topic/Democratic-Party). Because Democrats favor government programs like welfare, in theory it would make sense for low income voters to vote Democrat more than Republican.
...
We will only examine voting behavior of low income white Democrats and Republicans. There is not as much data on Independent voters compared to that of the two main parties, and different Independent candidates throughout the years do not necessarily share the same ideologies. Hence, it would be harder to narrow in on demographics of Independent voters throughout the years, so we will omit them from this analysis.
...
anes_use<-anes_dat%>%
  mutate(
    ...
    bible84=as_factor(VCF0850),
    churchat=as_factor(VCF0130),
    fundamental=as_factor(VCF0234),
    traditional=as_factor(VCF0853),
    aidpoor=as_factor(VCF0886),
    church.attendance=as_factor(VCF0130a),
...
First, let's do an income breakdown of all Democrat and Republican voters throughout the election years. Although there are some years where the very wealthy (96 to 100 percentile) have a smaller proportion of Republican voters compared to the other income groups, it looks like that for the most part, lower income Americans are more likely to vote Democrat and higher income Americans are more likely to vote Republican.
...
However, now let's look at what happens when we further break income levels down by race. It looks like income is not as related to voting behavior as had been indicated by the previous graphs. White Americans mostly vote Republican, and non white Americans mostly vote Democrat, regardless of income level. 

The results from the previous graphs most likely stem from the fact that there are significantly more white Americans than non white Americans who participated in the survey, which skewed the vote proportion toward white voter behavior.
...
This phenomenon of low income white Americans mostly Republican even if the Democratic party would theoretically offer more aid to them is a bit peculiar. To understand why this could be the case, we will examine the differences between white Republicans and low income white Democrats.

Note that the ANES data has significantly more white participants than people of color, especially before 1980, so for the rest of the analysis we will not focus on non white Americans because a lack of data means it will be harder to narrow in on demographics within non white Americans.


## Differences in Religion

Regardless of the party, most low income white Americans are Christian (Protestant or Catholic), but a significantly higher proportion of Republicans are Protestant compared to the proportion of Protestants among Democrats.
...
Of the low income white Americans who are Christian, a higher proportion of Republicans believe in a fundamentalist interpretation of the Bible compared to Democrats. Moreover, a significantly higher amount of Democrats do not believe in the Bible at all despite being Christian, indicating that Christianity plays a more important role in Republicans' lives than Democrats. Note that the three main answers for this survey question do not seem to capture all the possible feelings a voter can have toward the Bible, so I have kept in the Other/DK respondents as a valid response.

```{r bible analysis, fig.height=10, fig.width=25,echo=FALSE}
anes_bible84=anes_use%>%
  filter(!is.na(income) & !is.na(vote) & !is.na(race) & !is.na(bible84))%>%
  filter(vote!="7. Did not vote or voted but not for president (exc.1972)")%>%
  mutate(low_income=ifelse(income=="1. 0 to 16 percentile"|income=="2. 17 to 33 percentile","Low income","Medium to High income"))%>%
  mutate(is_white=ifelse(race=="1. White non-Hispanic (1948-2012)","White non-Hispanic (1948-2012)", "Not white"))%>%
  filter(is_white=="White non-Hispanic (1948-2012)")%>%
  ...
  filter(low_income=="Low income")%>%
  group_by(year,vote,bible84)%>%
  count(bible84)%>%
  group_by(year,vote)%>%
  mutate(prop=n/sum(n))

levels(anes_bible84$vote)<-substr(levels(anes_bible84$vote),4,nchar(levels(anes_bible84$vote)))
levels(anes_bible84$bible84)<-substr(levels(anes_bible84$bible84),4,nchar(levels(anes_bible84$bible84)))
levels(anes_bible84$bible84)=c('NA; form B (1990); short form or Spanish language','The Bible is the actual Word of God and is to be taken literally','The Bible is the Word of God but not everything in it should be taken literally','The Bible is a book written by men and is not the Word of God','Other; DK')

ggplot(anes_bible84, 
       aes(x=year, y=prop, fill=bible84)) +
  geom_bar(stat="identity", colour="black")+ 
...
Of the low income white Americans who are Christians, Republicans feel that Christianity has more guidance in their lives than it does for Democrats, which reinforces the

Of the low income white Americans who are Christians, Republicans have had higher church attendance than Democrats, which reinforces that Christianity plays a larger role in Republicans' lives.
...
anes_churchat=anes_use%>%
  filter(!is.na(income) & !is.na(vote) & !is.na(race) & !is.na(churchat))%>%
  filter(vote!="7. Did not vote or voted but not for president (exc.1972)")%>%
  filter(churchat!="0. Religion Inap (1970-1988); atheists and agnostics are" & churchat!="9. NA/RF how often/NA/RF if attend")%>%
  mutate(low_income=ifelse(income=="1. 0 to 16 percentile"|income=="2. 17 to 33 percentile","Low income","Medium to High income"))%>%
  mutate(is_white=ifelse(race=="1. White non-Hispanic (1948-2012)","White non-Hispanic (1948-2012)", "Not white"))%>%
  mutate(churchat=ifelse(churchat=="1. Every week (Except 1970: almost every week)",'At least once a week',ifelse(churchat=="2. Almost every week (no cases in 1970)"|churchat=='3. Once or twice a month', 'At least once a month',ifelse(churchat=='4. A few times a year','A few times a year','Never'))))%>%
  filter(is_white=="White non-Hispanic (1948-2012)")%>%
  filter(religion=='1. Protestant'|religion=='2. Catholic [Roman Catholic]')%>%
  filter(vote=="1. Democrat"|vote=="2. Republican")%>%
  filter(low_income=="Low income")%>%
  group_by(year,vote,churchat)%>%
  count(vote)%>%
  group_by(year,vote)%>%
  mutate(prop=n/sum(n))

anes_churchat$churchat=factor(anes_churchat$churchat,levels=c('At least once a week','At least once a month','A few times a year','Never'))
levels(anes_churchat$vote)<-substr(levels(anes_churchat$vote),4,nchar(levels(anes_churchat$vote)))



ggplot(anes_churchat, 
       aes(x=year, y=prop, fill=churchat)) +
  geom_bar(stat="identity", colour="black")+ 
...
## Differences in Views on Traditional Values and Government

Most Republicans agree that there should be more of an emphasis on a traditional family ties. Most Democrats shared this sentiment until the past decade. The survey does not explicitly say what traditional family ties refer to, but <>. 
...
Most low income white Republicans believe that the government has gotten too involved in things that people can do for themselves. On the other hand, most low income white Democrats believe that the problems we face now are bigger and the role the government plays in our lives has increased appropriately as a result. These beliefs match the ideologies of their respective party as well as the results of the previous graph.
...
In terms of specific government programs, more Republicans want welfare and other aid programs to low income Americans to stay the same/decrease compared to Democrats. The Republican belief falls in line with how they mostly want less government control in their lives, especially in the economy, so it looks like the belief in less government overrides their desire for an increase in financial security despite the fact that they have little income. 
...
In every election year since 1952, low income white Republicans have had a higher turnout rate than low income white Democrats. This difference in turnout may indicate that low income white Republicans are more politically active than low income white Democrats. 
...
In conclusion, this analysis indicates that low income white Americans mostly vote Republican, and low income non white Americans mostly vote Democrat.

Despite the fact that Democrats offer more government aid programs, most low income white Republicans emphasize family ties and a strong relationship with Christianity and their church to help with their everyday problems instead of the government.  

On the other hand, most low income white Democrats' are Christian as well, but religion plays less of a role in their lives, and they believe more in government aid like welfare programs.
